...
void writeJPEG(WG *wg, char *infile)
{
  uint8_t buf[128];
  struct stat statbuf;
  stat(infile, &statbuf);
  ...
  putData_String(wg, (char*)buf);
  putData_String(wg, "Content-Type:image/jpeg\n\n");
...
{
  FILE *fw = fopen("output_jpeg.wav", "wb");
  if (!fw) {
    perror("fopen");
    return -1;
  }
